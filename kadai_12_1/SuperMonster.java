...
public class SuperMonster extends Monster {
	...
	public void poison_Attack(Charactor cha) {
		if(getPoison() <= 0) {
			System.out.println("さらに"+this.getName() + "は毒針を発射しようとした！");
			System.out.println("だが、毒針は尽きている！");
			return;
		}
		System.out.println("さらに"+this.getName() + "は毒針を発射した！");
		System.out.println(cha.getName() + "は" + this.getP_at() + "ポイントのダメージを受けた");
